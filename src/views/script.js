...
async function makeAPICall(endpoint, options = {}) {
  try {
    ...
    const isDemoMode = API_BASE_URL.includes("localhost") || !navigator.onLine;
...
function getMockResponse(endpoint, options) {
  if (endpoint.includes("/stripe/deposit")) {
    return {
      clientSecret:
...
function showTab(id) {
  ...
  const tabButton = document.querySelector(`[onclick="showTab('${id}')"]`);
  if (tabButton) {
...
async function processWithdraw() {
  ...
  try {
    ...
    showAlert("An error occurred while processing the withdrawal.", "error");
  } finally {
...
async function processTransfer() {
  ...
  try {
    ...
    showAlert("An error occurred while processing the transfer.", "error");
  } finally {
...
async function initializeStripePayment(amount, email, name) {
  ...
  try {
    ...
    paymentElement.on("ready", () => {
      console.log("Payment Element ready" + (isDemoMode ? " (Demo Mode)" : ""));
    });
...
function searchTransactions() {
  ...
  const filteredTransactions = transactions.filter(
    (transaction) =>
      ...
      (transaction.toAccount && transaction.toAccount.includes(searchTerm))
  );
